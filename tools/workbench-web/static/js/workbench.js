let currentCandidate = null;
    ...
    async function refreshDrafts() {
      ...
      for (const v of draftVersions) {
        ...
        const label = reason.startsWith('apply:')
          ? `Edited ${key || reason.replace(/^apply:/, '')}`
          : reason;
        opt.textContent = `${when} — ${label}`;
        sel.appendChild(opt);
      }
    ...
    async function onDraftSelected() {
      ...
      try {
        ...
        const notes = [];
        notes.push('Edit snapshot (saved after Apply).');
        notes.push('Use Restore snapshot to load it back into Candidate.');
        notes.push('');
        notes.push(`id=${id}`);
        if (ver && ver.reason) notes.push(`reason=${String(ver.reason)}`);
        if (ver && ver.targetKey) notes.push(`targetKey=${String(ver.targetKey)}`);
        if (ver && ver.model) notes.push(`model=${String(ver.model)}`);
        if (ver && ver.traceId) notes.push(`traceId=${String(ver.traceId)}`);
        if (ver && ver.suite) notes.push('suite: ' + suiteLine(ver.suite));
        if (ver && ver.notes) {
          notes.push('');
          notes.push(String(ver.notes));
        }
    ...
    async function aiPropose() {
      ...
      status.textContent = ok ? 'OK' : 'Not applyable';
    }
